...
public class GroundNavigator extends GroundPathNavigation {
    ...
    private boolean catchF(Vec3 vec, Vec3 base) {
        ...
        while (t <= maxT) {
            ...
            PathType pathNodeType = this.nodeEvaluator.getPathType(new PathfindingContext(level, mob), currentX, currentY, currentZ);
            float malus = this.mob.getPathfindingMalus(pathNodeType);

            if (malus < 0.0F || malus >= 8.0F || pathNodeType == PathType.DAMAGE_FIRE || pathNodeType == PathType.DANGER_FIRE || pathNodeType == PathType.DAMAGE_OTHER)
                return false;

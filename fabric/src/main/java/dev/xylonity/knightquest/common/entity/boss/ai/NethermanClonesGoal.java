...
public class NethermanClonesGoal extends Goal {
            ...
            if (livingentity.distanceToSqr(this.netherman) < 4096.0D && this.netherman.hasLineOfSight(livingentity)) {
                ...
                if (this.chargeTime == 10) {
                    ...
                    for (int i = 0; i < KQConfigValues.MAX_NETHERMAN_CLONES; i++) {
                        ...
                        NethermanCloneEntity nethermanClone = KnightQuestEntities.NETHERMAN_CLONE.create(this.netherman.level());
    ...
    private boolean isSafePosition(Vec3 pos, Level level) {
        NethermanCloneEntity test = KnightQuestEntities.NETHERMAN_CLONE.create(level);
        if (test != null) {

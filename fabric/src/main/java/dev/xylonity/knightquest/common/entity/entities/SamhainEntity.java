...
public class SamhainEntity extends TamableAnimal implements GeoEntity, RangedAttackMob {
    ...
    public void performRangedAttack(@NotNull LivingEntity target, float velocity) {
        InteractionHand hand = getWeaponHoldingHand(this, Items.BOW);
        ItemStack bowItemStack = this.getItemInHand(hand);
        ItemStack arrowItemStack = this.getProjectile(bowItemStack);

        AbstractArrow arrowEntity = createArrowEntity(arrowItemStack, velocity);
        if (bowItemStack.getItem() instanceof BowItem) {
            arrowEntity = ((ArrowItem) (arrowItemStack.getItem() instanceof ArrowItem ? arrowItemStack.getItem() : Items.ARROW))
                    .createArrow(this.level(), arrowItemStack, this);
        }

        double deltaX = target.getX() - this.getX();
        double deltaY = target.getY(0.3333333333333333D) - arrowEntity.getY();
        double deltaZ = target.getZ() - this.getZ();
        double distance = Math.sqrt(deltaX * deltaX + deltaZ * deltaZ);

        arrowEntity.shoot(deltaX, deltaY + distance * 0.2D, deltaZ, 1.6F, (float) (14 - this.level().getDifficulty().getId() * 4));
        this.playSound(SoundEvents.SKELETON_SHOOT, 1.0F, 1.0F / (this.getRandom().nextFloat() * 0.4F + 0.8F));
        this.level().addFreshEntity(arrowEntity);
    }

    protected AbstractArrow createArrowEntity(ItemStack arrowStack, float velocity) {
        AbstractArrow arrowEntity;
        if (arrowStack.getItem() instanceof ArrowItem arrowItem) {
            arrowEntity = arrowItem.createArrow(this.level(), arrowStack, this);
        } else {
            arrowEntity = new Arrow(this.level(), this);
        }
        arrowEntity.setEnchantmentEffectsFromEntity(this, velocity);
        return arrowEntity;
    }

...
public class KnightQuestEntities {
    ...
    public static final EntityType<MommaLizzyEntity> MOMMA_LIZZY;
    ...
    private static <X extends Entity> EntityType<X> register(String name, EntityType.EntityFactory<X> entity, MobCategory spawnGroup, float width, float height) {
        return Registry.register(BuiltInRegistries.ENTITY_TYPE, ResourceLocation.fromNamespaceAndPath(KnightQuest.MOD_ID, name), EntityType.Builder.of(entity, spawnGroup).sized(width, height).build(name));
    }
    ...
    private static <X extends Mob> Item registerSpawnEggItem(String id, EntityType<X> entityType, int primaryEggColour, int secondaryEggColour) {
        return Registry.register(BuiltInRegistries.ITEM, ResourceLocation.fromNamespaceAndPath(KnightQuest.MOD_ID, id), new SpawnEggItem(entityType, primaryEggColour, secondaryEggColour, new Item.Properties()));
    }

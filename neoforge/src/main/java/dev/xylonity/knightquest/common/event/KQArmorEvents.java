    ...
    public static class ArmorStatusManagerEvents {
        ...
        public static void onLivingHurt(LivingIncomingDamageEvent event) {
            ...
            if (event.getEntity() instanceof Player player) {
                ...
                if (KQConfigValues.DEEPSLATESET.getBoolean())
                    if (event.getSource().is(DamageTypes.FALL) && hasFullSetOn(player, KQArmorMaterials.DEEPSLATESET)) {
                        float originalDamage = event.getAmount();
                        float reducedDamage = originalDamage * KQConfigValues.DEEPSLATE_FALL_DAMAGE_MULTIPLIER.getFloat();
                ...
                if (KQConfigValues.EVOKERSET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.EVOKERSET)) {
                        ...
                        if (event.getSource().getEntity() != null && event.getSource().getEntity() instanceof LivingEntity entity && random.nextFloat() < KQConfigValues.EVOKER_DARKNESS_CHANCE.getFloat())
                            entity.addEffect(new MobEffectInstance(MobEffects.DARKNESS, 120, 0, false, false, true));
                    }

                if (KQConfigValues.SQUIRESET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.SQUIRESET)) {
                        event.setAmount(event.getAmount() * KQConfigValues.SQUIRE_DAMAGE_RECEIVED_MULTIPLIER.getFloat());
                    }

                if (KQConfigValues.BLAZESET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.BLAZESET)) {
                        Random random = new Random();
                        if (event.getSource().getEntity() != null && random.nextFloat() < KQConfigValues.BLAZE_FIRE_CHANCE.getFloat())
                            event.getSource().getEntity().setRemainingFireTicks(random.nextInt(KQConfigValues.BLAZE_FIRE_DURATION_MIN.getInt(), KQConfigValues.BLAZE_FIRE_DURATION_MAX.getInt()) * 20);
                    }
                ...
                if (KQConfigValues.BAMBOOSET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.BAMBOOSET) && event.getSource().is(DamageTypes.FALL) ) {
                        ...
                        Class<? extends Entity> classToPush = KQConfigValues.BAMBOOSET_PUSH_PLAYERS.getBoolean() ? Entity.class : Monster.class;
                ...
                if (KQConfigValues.FORZESET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.FORZESET)) {
                        Random random = new Random();
                        if (event.getSource().getEntity() != null && random.nextFloat() < KQConfigValues.FORZESET_DEFLECT_CHANCE.getFloat())
                            event.getSource().getEntity().hurt(event.getSource(), event.getAmount() * KQConfigValues.FORZESET_DEFLECT_DAMAGE.getFloat());
                    }

                if (KQConfigValues.CREEPERSET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.CREEPERSET)) {
                        if (event.getSource().getEntity() != null && (event.getSource().is(DamageTypes.EXPLOSION) || event.getSource().is(DamageTypes.PLAYER_EXPLOSION)))
                            event.setAmount(event.getAmount() * KQConfigValues.CREEPER_EXPLOSION_DAMAGE_MULTIPLIER.getFloat());
                    }
            ...
            if (event.getSource().getEntity() instanceof Player player) {
                ...
                if (KQConfigValues.HOLLOWSET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.HOLLOWSET))
                        player.heal(Math.min((event.getAmount() * KQConfigValues.HOLLOWSET_HEALING_MULTIPLIER.getFloat()), event.getEntity().getHealth()));

                if (KQConfigValues.DRAGONSET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.DRAGONSET))
                        event.setAmount(event.getAmount() * KQConfigValues.DRAGONSET_DAMAGE_MULTIPLIER.getFloat());
                ...
                if (KQConfigValues.WITHERSET.getBoolean())
                    if (event.getSource().is(DamageTypes.ARROW) && hasFullSetOn(player, KQArmorMaterials.WITHERSET)) {
                        Random random = new Random();
                        if (event.getSource().getEntity() != null && random.nextFloat() < KQConfigValues.WITHERSET_WITHER_CHANCE.getFloat())
        ...
        public static void onLivingTick(EntityTickEvent.Post event) {
            if (event.getEntity() instanceof Player player) {
                ...
                if (KQConfigValues.TENGU_HELMET.getBoolean())
                ...
                if (KQConfigValues.SILVERFISHSET.getBoolean())
                    if (hasFullSetOn(player, KQArmorMaterials.SILVERFISHSET) && player.getY() < KQConfigValues.SILVERFISH_EFFECT_MAX_HEIGHT.getInt()) {

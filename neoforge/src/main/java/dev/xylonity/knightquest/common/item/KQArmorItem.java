...
public class KQArmorItem extends ArmorItem {
    ...
    public KQArmorItem(Holder<ArmorMaterial> material, Type type, Properties settings, boolean containsTooltip) {
        super(material, type, settings.stacksTo(1));
        this.bonusTooltip = KQArmorMaterials.getKeyNameFromMaterial(material);
        this.hasTooltip = containsTooltip;
    }
    ...
    public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected) {
        ...
        if(!pLevel.isClientSide() && pEntity instanceof Player player) {
            ...
            if (KQConfigValues.PATHSET)
                if (hasFullSetOn(player, KQArmorMaterials.PATHSET.get()) && player.isShiftKeyDown()) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.PATHSET.get(), false))) {
                        player.addEffect(PATH_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.PATHSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.PATHSET.get(), false))) {
                        player.removeEffect(MobEffects.MOVEMENT_SPEED);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.PATHSET.get(), false);
                    }
                }

            if (KQConfigValues.BOWSET)
                if (hasFullSetOn(player, KQArmorMaterials.BOWSET.get()) && player.getMainHandItem().getItem() instanceof ProjectileWeaponItem) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.BOWSET.get(), false))) {
                        player.addEffect(BOW_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.BOWSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.BOWSET.get(), false))) {
                        player.removeEffect(MobEffects.MOVEMENT_SPEED);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.BOWSET.get(), false);
                    }
                }

            if (KQConfigValues.BATSET)
                if (hasFullSetOn(player, KQArmorMaterials.BATSET.get()) && pLevel.isNight()) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.BATSET.get(), false))) {
                        player.addEffect(BAT_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.BATSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.BATSET.get(), false))) {
                        player.removeEffect(MobEffects.NIGHT_VISION);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.BATSET.get(), false);
                    }
                }

            if (KQConfigValues.SHIELDSET)
                if (hasFullSetOn(player, KQArmorMaterials.SHIELDSET.get())) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SHIELDSET.get(), false))) {
                        player.addEffect(SHIELD_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SHIELDSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SHIELDSET.get(), false))) {
                        player.removeEffect(MobEffects.DAMAGE_RESISTANCE);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SHIELDSET.get(), false);
                    }
                }

            if (KQConfigValues.PHANTOMSET)
                if (hasFullSetOn(player, KQArmorMaterials.PHANTOMSET.get()) && pLevel.isNight()) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.PHANTOMSET.get(), false))) {
                        player.addEffect(PHATOM_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.PHANTOMSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.PHANTOMSET.get(), false))) {
                        player.removeEffect(MobEffects.MOVEMENT_SPEED);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.PHANTOMSET.get(), false);
                    }
                }

            if (KQConfigValues.HORNSET)
                if (hasFullSetOn(player, KQArmorMaterials.HORNSET.get()) && player.getLastAttacker() != null) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.HORNSET.get(), false))) {
                        player.addEffect(HORN_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.HORNSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.HORNSET.get(), false))) {
                        player.removeEffect(MobEffects.DAMAGE_BOOST);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.HORNSET.get(), false);
                    }
                }

            if (KQConfigValues.SEASET)
                if (hasFullSetOn(player, KQArmorMaterials.SEASET.get()) && player.isUnderWater()) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SEASET.get(), false))) {
                        player.addEffect(SEA_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SEASET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SEASET.get(), false))) {
                        player.removeEffect(MobEffects.DOLPHINS_GRACE);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SEASET.get(), false);
                    }
                }

            if (KQConfigValues.PIRATESET)
                if (hasFullSetOn(player, KQArmorMaterials.PIRATESET.get())) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.PIRATESET.get(), false))) {
                        player.addEffect(PIRATE_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.PIRATESET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.PIRATESET.get(), false))) {
                        player.removeEffect(MobEffects.LUCK);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.PIRATESET.get(), false);
                    }
                }

            if (KQConfigValues.SPIDERSET)
                if (hasFullSetOn(player, KQArmorMaterials.SPIDERSET.get()) && player.isShiftKeyDown()) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SPIDERSET.get(), false))) {
                        player.addEffect(SPIDER_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SPIDERSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SPIDERSET.get(), false))) {
                        player.removeEffect(MobEffects.JUMP);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SPIDERSET.get(), false);
                    }
                }

            if (KQConfigValues.NETHERSET)
                if (hasFullSetOn(player, KQArmorMaterials.NETHERSET.get())) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.NETHERSET.get(), false))) {
                        player.addEffect(NETHER_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.NETHERSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.NETHERSET.get(), false))) {
                        player.removeEffect(MobEffects.FIRE_RESISTANCE);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.NETHERSET.get(), false);
                    }
                }

            if (KQConfigValues.SKULK)
                if (hasFullSetOn(player, KQArmorMaterials.SKULK.get()) && player.level().getMaxLocalRawBrightness(player.blockPosition()) <= KQConfigValues.SKULK_MAX_LIGHT_LEVEL) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SKULK.get(), false))) {
                        player.addEffect(SKULK_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SKULK.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SKULK.get(), false))) {
                        player.removeEffect(MobEffects.DAMAGE_RESISTANCE);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.SKULK.get(), false);
                    }
                }

            if (KQConfigValues.STRAWHATSET)
                if (hasFullSetOn(player, KQArmorMaterials.STRAWHATSET.get()) && player.isUnderWater()) {
                    if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.STRAWHATSET.get(), false))) {
                        player.addEffect(STRAWHAT_ARMOR);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.STRAWHATSET.get(), true);
                    }
                } else {
                    if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(playerUUID, k -> new HashMap<>()).getOrDefault(KQArmorMaterials.STRAWHATSET.get(), false))) {
                        player.removeEffect(MobEffects.WATER_BREATHING);
                        effectAppliedByArmorMap.get(playerUUID).put(KQArmorMaterials.STRAWHATSET.get(), false);
                    }
                }
        ...
        public static void onLivingHurt(LivingIncomingDamageEvent event) {
            ...
            if (event.getEntity() instanceof Player player) {
                ...
                if (KQConfigValues.DEEPSLATESET)
                    if (event.getSource().is(DamageTypes.FALL) && KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.DEEPSLATESET.get())) {
                        float originalDamage = event.getAmount();
                        float reducedDamage = originalDamage * (float) KQConfigValues.DEEPSLATE_FALL_DAMAGE_MULTIPLIER;
                        event.setAmount(reducedDamage);
                    }

                if (KQConfigValues.EVOKERSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.EVOKERSET.get())) {
                        Random random = new Random();
                        if (event.getSource().getEntity() != null && event.getSource().getEntity() instanceof LivingEntity entity && random.nextFloat() < KQConfigValues.EVOKER_DARKNESS_CHANCE)
                            entity.addEffect(new MobEffectInstance(MobEffects.DARKNESS, 120, 0, false, false, true));
                    }

                if (KQConfigValues.SQUIRESET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.SQUIRESET.get())) {
                        event.setAmount(event.getAmount() * (float) KQConfigValues.SQUIRE_DAMAGE_RECEIVED_MULTIPLIER);
                    }

                if (KQConfigValues.BLAZESET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.BLAZESET.get())) {
                        Random random = new Random();
                        if (event.getSource().getEntity() != null && random.nextFloat() < KQConfigValues.BLAZE_FIRE_CHANCE)
                            event.getSource().getEntity().setRemainingFireTicks(random.nextInt(KQConfigValues.BLAZE_FIRE_DURATION_MIN, KQConfigValues.BLAZE_FIRE_DURATION_MAX) * 20);
                    }

                if (KQConfigValues.DRAGONSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.DRAGONSET.get()))
                        if (event.getSource().is(DamageTypes.DRAGON_BREATH))
                            event.setAmount(0);

                if (KQConfigValues.BAMBOOSET_GREEN)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.BAMBOOSET_GREEN.get()))
                ...
                if (KQConfigValues.SHINOBI)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.SHINOBI.get()) && event.getSource().getEntity() != null) {
                        player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 120, 1, false, false, true));
                    }

                if (KQConfigValues.BAMBOOSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.BAMBOOSET.get()) && event.getSource().is(DamageTypes.FALL) ) {
                ...
                if (KQConfigValues.ENDERMANSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.ENDERMANSET.get()) && event.getSource().getEntity() != null) {
                ...
                if (KQConfigValues.VETERANSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.VETERANSET.get()) && player.getHealth() < player.getMaxHealth() * 0.5) {
                ...
                if (KQConfigValues.FORZESET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.FORZESET.get())) {
                        Random random = new Random();
                ...
                if (KQConfigValues.CREEPERSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.CREEPERSET.get())) {
                        if (event.getSource().getEntity() != null && (event.getSource().is(DamageTypes.EXPLOSION) || event.getSource().is(DamageTypes.PLAYER_EXPLOSION)))
                            event.setAmount(event.getAmount() * (float) KQConfigValues.CREEPER_EXPLOSION_DAMAGE_MULTIPLIER);
                    }

                if (KQConfigValues.POLAR)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.POLAR.get())) {
            ...
            if (event.getSource().getEntity() instanceof Player player && event.getEntity() != null) {
                ...
                if (KQConfigValues.SILVERSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.SILVERSET.get()) && player.level().isNight()) {
                        Random random = new Random();
                ...
                if (KQConfigValues.HOLLOWSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.HOLLOWSET.get()))
                ...
                if (KQConfigValues.WITHERSET)
                    if (event.getSource().is(DamageTypes.ARROW) && KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.WITHERSET.get())) {
                        Random random = new Random();
        ...
        public static void onLivingUpdate(LivingEntityUseItemEvent.Finish event) {
            if (event.getEntity() instanceof Player player)
                if (KQConfigValues.APPLE_SET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.APPLE_SET.get()))
        ...
        public static void onLivingDead(LivingDeathEvent event) {
            if (event.getEntity() != null && event.getSource().getEntity() instanceof Player player) {
                ...
                if (KQConfigValues.CONQUISTADORSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.CONQUISTADORSET.get())) {
                        player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 200, 1, false, true, true));
                    }

                if (KQConfigValues.WITCH)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.WITCH.get())) {
        ...
        public static void onLivingTick(EntityTickEvent.Post event) {
            if (event.getEntity() instanceof Player player) {
                ...
                if (KQConfigValues.HUSKSET)
                    if (hasFullSetOn(player, KQArmorMaterials.HUSKSET.get()) && (player.level().getBiome(new BlockPos((int) player.getX(), (int) player.getY(), (int) player.getZ())).is(Biomes.DESERT)
                        ...
                        if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(player.getUUID(), k -> new HashMap<>()).getOrDefault(KQArmorMaterials.HUSKSET.get(), false))) {
                            player.addEffect(HUSK_ARMOR);
                            effectAppliedByArmorMap.get(player.getUUID()).put(KQArmorMaterials.HUSKSET.get(), true);
                        }
                    } else {
                        if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(player.getUUID(), k -> new HashMap<>()).getOrDefault(KQArmorMaterials.HUSKSET.get(), false))) {
                            player.removeEffect(MobEffects.DAMAGE_RESISTANCE);
                            effectAppliedByArmorMap.get(player.getUUID()).put(KQArmorMaterials.HUSKSET.get(), false);
                        }
                    }

                if (KQConfigValues.BAMBOOSET_BLUE)
                    if (hasFullSetOn(player, KQArmorMaterials.BAMBOOSET_BLUE.get()) && (player.level().getBiome(new BlockPos((int) player.getX(), (int) player.getY(), (int) player.getZ())).is(Biomes.JUNGLE)
                        ...
                        if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(player.getUUID(), k -> new HashMap<>()).getOrDefault(KQArmorMaterials.BAMBOOSET_BLUE.get(), false))) {
                            player.addEffect(BAMBOO_BLUE);
                            effectAppliedByArmorMap.get(player.getUUID()).put(KQArmorMaterials.BAMBOOSET_BLUE.get(), true);
                        }
                    } else {
                        if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(player.getUUID(), k -> new HashMap<>()).getOrDefault(KQArmorMaterials.BAMBOOSET_BLUE.get(), false))) {
                            player.removeEffect(MobEffects.MOVEMENT_SPEED);
                            effectAppliedByArmorMap.get(player.getUUID()).put(KQArmorMaterials.BAMBOOSET_BLUE.get(), false);
                        }
                    }

                if (KQConfigValues.WARLORDSET)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.WARLORDSET.get())) {
                ...
                if (KQConfigValues.ZOMBIESET)
                    if (!player.level().isClientSide) {
                        if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.ZOMBIESET.get()) && player.level().isNight()) {
                ...
                if (KQConfigValues.POLAR)
                    if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.POLAR.get()) && player.getTicksFrozen() > 0)
                        player.setTicksFrozen(0);

                if (KQConfigValues.SILVERFISHSET)
                    if (hasFullSetOn(player, KQArmorMaterials.SILVERFISHSET.get()) && player.getY() < KQConfigValues.SILVERFISH_EFFECT_MAX_HEIGHT) {
                        if (!Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(player.getUUID(), k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SILVERFISHSET.get(), false))) {
                            player.addEffect(SILVERFISH_ARMOR);
                            effectAppliedByArmorMap.get(player.getUUID()).put(KQArmorMaterials.SILVERFISHSET.get(), true);
                        }
                    } else {
                        if (Boolean.TRUE.equals(effectAppliedByArmorMap.computeIfAbsent(player.getUUID(), k -> new HashMap<>()).getOrDefault(KQArmorMaterials.SILVERFISHSET.get(), false))) {
                            player.removeEffect(MobEffects.DIG_SPEED);
                            effectAppliedByArmorMap.get(player.getUUID()).put(KQArmorMaterials.SILVERFISHSET.get(), false);
                        }
                    }
            }
        }
        ...
        public static void onArrowHit(EntityJoinLevelEvent event) {
            if (event.getEntity() instanceof AbstractArrow arrow) {
                if (KQConfigValues.SKELETONSET)
                    if (arrow.getOwner() instanceof Player player && KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.SKELETONSET.get())) {

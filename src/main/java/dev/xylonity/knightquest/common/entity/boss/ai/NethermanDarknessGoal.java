            ...
            if (livingentity.distanceToSqr(this.netherman) < 4096.0D && this.netherman.hasLineOfSight(livingentity)) {
                ...
                if (chargeTime == 5) {
                    ...
                    for (Player player : this.netherman.level().players()) {
                        ...
                        player.addEffect(new MobEffectInstance(MobEffects.DARKNESS, this.netherman.getRandom().nextInt(KQConfigValues.DARKNESS_ATTACK_MIN_TIME.get(), KQConfigValues.DARKNESS_ATTACK_MAX_TIME.get()) * 20, 0, false, false));

...
public class KQArmorItem extends ArmorItem {
    ...
    public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected) {
        ...
        if(!pLevel.isClientSide() && pEntity instanceof Player player) {
            ...
            UUID playerUUID = pEntity.getUUID();
            ...
            if (KQConfigValues.SKULK)
                if (KQFullSetChecker.hasFullSetOn(player, KQArmorMaterials.SKULK) && player.level.getMaxLocalRawBrightness(player.blockPosition()) <= 4) {
    ...
    public static class ArmorStatusManagerEvents {
        ...
        public static void onLivingHurt(LivingHurtEvent event) {
            ...
            if (event.getSource().getEntity() instanceof Player player && event.getEntity() != null && event.getEntity() instanceof LivingEntity entity) {
                ...
                if (KQConfigValues.HOLLOWSET)
                    if (KQFullSetChecker.hasFullSuitOfArmorOn(player, KQArmorMaterials.HOLLOWSET))
                        player.heal(Math.min((float) (event.getAmount() * 0.25), entity.getHealth()));
